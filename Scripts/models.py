...
class HN(nn.Module):
    def __init__(
        ...
        self.t_conv2_weights = nn.Linear(
            hidden_dim, self.in_channels * self.n_kernels * self.n_kernels_dec * self.n_kernels_dec)
    ...
    def forward(self, idx):
        ...
        weights = OrderedDict({
            "conv1.weight": self.conv1_weights(features).view(self.n_kernels, 1, self.n_kernels_enc, self.n_kernels_enc),
            "conv1.bias": self.conv1_bias(features).view(-1),
            "conv2.weight": self.conv2_weights(features).view(self.latent_rep, self.n_kernels, self.n_kernels_enc, self.n_kernels_enc),
...
class Autoencoder(nn.Module):
    def __init__(self, inout_channels=1, hidden=16, n_kernels_enc=3,
        ...
        self.conv2 = nn.Conv2d(
            hidden, 8, n_kernels_enc, padding=padding_value)
...
#            padding=padding_value)
        ...
        self.t_conv1 = nn.ConvTranspose2d(
            latent_rep, 8, kernel_size=(n_kernels_dec, n_kernels_dec), stride=stride_value, padding=padding_value)
        self.t_conv2 = nn.ConvTranspose2d(
            8, hidden, kernel_size=(n_kernels_dec, n_kernels_dec), stride=stride_value, padding=padding_value)
...
class BASEModel(nn.Module):
    def __init__(self, latent_rep=9 * 30 * 4, out_dim=20, hidden_layer=128):
